...
class PhieuThueXeController {
    ...
    getPhieuThueXeById = async (req, res, next) => {
        const phieuThueXe = await PhieuThueXeModel.findOne({ ma_phieu_thue_xe: req.params.ma_phieu_thue_xe });
        if (!phieuThueXe) {
            throw new HttpException(404, 'PhieuThueXe not found');
        }
    ...
    getPhieuThueXeByMaNguoiThue = async (req, res, next) => {
        const phieuThueXe = await PhieuThueXeModel.find({ ma_nguoi_thue: req.params.ma_nguoi_thue });
        if (!phieuThueXe) {
            throw new HttpException(404, 'PhieuThueXe not found');
        }
        res.send(phieuThueXe);
    ...
    getPhieuThueXeByNgayThue = async (req, res, next) => {
        const phieuThueXe = await PhieuThueXeModel.find({ ngay_thue: req.params.ngay_thue });
        if (!phieuThueXe) {
            throw new HttpException(404, 'PhieuThueXe not found');
        }
        res.send(phieuThueXe);
    ...
    getPhieuThueXeByNgayTra = async (req, res, next) => {
        const phieuThueXe = await PhieuThueXeModel.find({ ngay_tra: req.params.ngay_tra });
        if (!phieuThueXe) {
            throw new HttpException(404, 'PhieuThueXe not found');
        }
        res.send(phieuThueXe);
    ...
    getPhieuThueXeByTrangThai = async (req, res, next) => {
        const phieuThueXe = await PhieuThueXeModel.find({ trang_thai: req.params.trang_thai });
        if (!phieuThueXe) {
            throw new HttpException(404, 'PhieuThueXe not found');
        }
        res.send(phieuThueXe);
    ...
    createPhieuThueXe = async (req, res, next) => {
        ...
        res.status(201).send('PhieuThueXe was created!');
    };

    updatePhieuThueXe = async (req, res, next) => {
        const result = await PhieuThueXeModel.update(restOfUpdates, req.params.ma_phieu_thue_xe);
        if (!result) {
        ...
        const message = !affectedRows ? 'PhieuThueXe not found' :
            affectedRows && changedRows ? 'PhieuThueXe updated successfully' : 'Updated faild';
